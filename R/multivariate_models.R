...
calc_neg_log_lik_vect_multivariate <- function(th_y,calcData,tfCatVect=NA) {
  ...
  negLogLikVect <- foreach(n=1:N,.combine=cbind,.packages=c('yada')) %dopar% {
...
calc_neg_log_lik_vect_multivariate_chunk_outer <- function(th_y,calcData,tfCatVect=NA,numChunks=round(length(calcData)/10)) {
  ...
  negLogLikVect <- foreach(k=1:numChunks,.combine=c,.packages=c('yada')) %dopar% {
...
#' @param Y The matrix of responses
#' @param modSpec The model specification
#' @param optimControl Control information for the call to doGpBasedOptim
#' @return A list object of simulated data containing x, Y, and Ystar
#' @export
fit_multivariate <- function(x,Y,modSpec,optimControl=list()) {
  ...
  if( !('scaleVect' %in% names(optimControl)) ) {
    # If the scale vector is not specified, set it
    Np <- length(th_y0)
    optimControl$scaleVect <- c(rep(5e-5,Np-Nz),rep(5e-3,Nz))
  }


  # Solve the optimization problem
  optimOutput <- doGpBasedOptim(th_y_bar0, objFun=calc_neg_log_lik_multivariate,control=optimControl,calcData=calcData,tfCatVect=tfCatVect)
...
#'   Default:    10000
#' *****************************************************************************
#' objTol      Tolerance to use in the stopping condition
#'   Default:    1e-4
#' *****************************************************************************
  ...
  for(s in s0:(s0+control$maxEval-1)) {
    ...
    if(is.finite(f_prop)) {
      ...
      if(nrow(paramMat) == control$Nmodel) {
        ...
        objVect  <- rbind( objVect[2:control$Nmodel] ,f_prop)
