...
#' for out-of-sample cross-validation or Akaike's Information Criterion.
#' 
#' @description Function that loads the cindep and cdep models folds,  
#' performs negative-log-likelihood calculations, calculates AIC if defined, 
#' and returns either the maximum likelihood ("cv") or aic for each model.
#' 
#' @param data_dir Directory where the problem and model files are saved
#' @param analysis_name Unique identifier for current analysis
#' @param fold Fold number, if needed for cross-validation. (Default is NA 
#'     for AIC)
#' 
#' @export
calc_eta_multivariate <- function(data_dir, analysis_name, fold=NA) {
  ...
  #       and test folds.
  if (is.na(fold)) {
    eta_cindep <- calc_neg_log_lik_multivariate(th_y_cindep, 
                                                calc_data_cindep)
    eta_cdep <- calc_neg_log_lik_multivariate(th_y_cdep,
                                              calc_data_cdep)
  } else {
    eta_cindep <- calc_neg_log_lik_vect_multivariate(th_y_cindep,
                                                     calc_data_cindep)
    eta_cdep   <- calc_neg_log_lik_vect_multivariate(th_y_cdep,
                                                     calc_data_cdep)
  }
  

  return(list(eta_cindep=eta_cindep,
              eta_cdep=eta_cdep))
}
...
#' @title Evaluation of multivariate models
#' 
#' @description [evaluate_multivariate_models] utilizes either out-of-sample
#' cross-validation ("cv") or Akaike's Information Criterion ("aic") to 
#' aid in multivariate model selection between the conditionally independent 
#' ("cindep") and conditionally dependent ("cdep") models generated by yada.
#' 
#' @param data_dir Directory where the problem and model files are saved
#' @param analysis_name Unique identifier for current analysis
#' @param eval_type Whether model selection should use out-of-sample 
#'     cross-validation ("cv") or Akaike's Information Criterion ("aic")
#' 
#' @export
evaluate_multivariate_models <- function(data_dir, analysis_name, 
                                         eval_type="aic") {
  if (!(eval_type %in% c("aic","cv"))) {
    stop("Not an available evaluation type.")
  }
  
  if (eval_type == "cv") {
    # Calculate maximum likelihood estimations using out-of-sample 
    # cross-validation folds
    fold_data_list <- list()
    num_folds <- get_num_training_problems(data_dir,analysis_name)
    for (fold in 1:num_folds) {
      fold_data_list[[fold]] <- calc_eta_multivariate(data_dir,
                                                      analysis_name,
                                                      fold)
    }
    ...
    eval_cindep <-
      sum(unlist(lapply(fold_data_list,
                        function(fold_data){sum(fold_data$eta_cindep)})))
    eval_cdep <-
      sum(unlist(lapply(fold_data_list,
                        function(fold_data){sum(fold_data$eta_cdep)})))
  } else {
    # Calculate AIC using negative log-likelihoods
    eta_list <- calc_eta_multivariate(data_dir, analysis_name)
    cindep_model <- readRDS(build_file_path(data_dir,
                                            analysis_name,
                                            "cindep_model"))
    k_cindep <- length(cindep_model$th_y)
    cdep_model <- readRDS(build_file_path(data_dir,
                                          analysis_name,
                                          "cdep_model"))
    k_cdep <- length(cdep_model$th_y)
    eval_cindep <- calc_aic(k_cindep, eta_list$eta_cindep)
    eval_cdep <- calc_aic(k_cdep, eta_list$eta_cdep)
  }
  
  return(list(eval_type=eval_type,
              eval_cindep=eval_cindep,
              eval_cdep=eval_cdep))
}
